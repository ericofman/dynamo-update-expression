...
function diff(original, modified, orphans = false) {
    ...
    const removedLeafNodes = _.differenceWith(originalLeafNodes, modifiedNodes, (a, b) => a.stringPath === b.stringPath && !nullified(a, b) && !emptied(a, b));

    const updatedLeafNodes = _.intersectionWith(modifiedLeafNodes, originalLeafNodes, (a, b) => a.stringPath === b.stringPath && a.value !== b.value && !nullified(b, a) && !emptied(b, a));
...
function allNodes(data) {
    return jp.nodes(data, '$..*').map(({ path, value }) => ({ path: path, stringPath: jp.stringify(path), value })).sort(sortBy('stringPath'));
}
