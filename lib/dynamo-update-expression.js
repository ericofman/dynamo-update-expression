...
function version({ currentVersion, newVersion, useCurrent = true, versionPath = '$.version', condition = '=', aliasContext = {} }) {
    ...
    const expectedVersionNode = alias({ path: versionPath, value: expectedVersion }, conditionExpression.ExpressionAttributeNames, expectedVersion !== undefined ? conditionExpression.ExpressionAttributeValues : undefined, // else: no need for ExpressionAttributeValues
    {
...
function getVersionedUpdateExpression({ original = {}, modified = {}, versionPath = '$.version', useCurrent = true, currentVersion, condition = '=', orphans = false, aliasContext = { truncatedAliasCounter: 1 } }) {
    let updateExpression = getUpdateExpression({ original, modified, orphans, aliasContext: (0, _extends3.default)({}, aliasContext, { prefix: '' }) });
...
function getUpdateExpression({ original, modified, ignoreDeletes = false, orphans = false, supportSets = false, aliasContext = { truncatedAliasCounter: 1 } }) {
    ...
    updateExpression.UpdateExpression = [setExp, removeExp, deleteExp].reduce((acc, value) => {
...
function alias(node, nameMap, valueMap, aliasContext = {}) {
    ...
    const parts = node.path.slice(1) // skip `$` part of the path
...
function diff(original, modified, orphans = false) {
    ...
    const removedLeafNodes = _.differenceWith(originalLeafNodes, modifiedNodes, (a, b) => a.path === b.path && !nullified(a, b) && !emptied(a, b));

    const updatedLeafNodes = _.intersectionWith(modifiedLeafNodes, originalLeafNodes, (a, b) => a.path === b.path && a.value !== b.value && !nullified(b, a) && !emptied(b, a));
...
function allNodes(data) {
    return jp.nodes(data, '$..*').map(({ path, value }) => ({ path: jp.stringify(path), value })).sort(sortBy('path'));
}
