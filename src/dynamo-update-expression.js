...
function getUpdateExpression({
  ...
  function removeExpression(removes) {
    const paths = removes.map(
      (node) => alias(node, updateExpression.ExpressionAttributeNames, undefined, aliasContext).path
    )
    if (paths.length === 0) {
      return
    }
  ...
  function setExpression(addOrUpdates) {
    const pairs = addOrUpdates
      .map((node) =>
        alias(node, updateExpression.ExpressionAttributeNames, updateExpression.ExpressionAttributeValues, aliasContext)
      )
  ...
  updateExpression.UpdateExpression = [setExp, removeExp, deleteExp].reduce(
    (acc, value) => (value ? (acc ? `${acc} ${value}` : `${value}`) : acc),
    ''
  )
...
function partitionedDiff(original, modified, orphans = false, supportSets = false) {
  ...
  const [_DELETE, _REMOVE] = supportSets
    ? _.partition(
        DELETE,
        (node) => regex.isNumericSubscript$.test(node.path) && (_.isNumber(node.value) || _.isString(node.value)) // @TODO: Support Node Buffer and/or ArrayBuffer sets?
      )
    : [[], DELETE]
...
function diff(original, modified, orphans = false) {
  ...
  const removedLeafNodes = ancestorNodes(
    _.differenceWith(
      originalNodes,
      modifiedNodes,
...
function sortBy(sortBy, mapping = (v) => v) {
  return (a, b) =>
    Number(mapping(a[sortBy]) > mapping(b[sortBy])) || Number(mapping(a[sortBy]) === mapping(b[sortBy])) - 1
}
